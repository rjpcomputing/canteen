	...
	.component( "customersTable",
		{
			...
			controller: ["Customer", "Event", "UI", function( Customer, Event, UI ) {
				let GetCustomerTypes = () => UI.customertypes( ( data ) => this.customerTypes = data.customer_type ).$promise;
				...
				this.EditCustomer = ( customer ) => {
					...
					Customer.get( { id: customer.id } ).$promise
						.then( ( _cust ) => Customer.save( { id: currentCustomer.id }, currentCustomer ).$promise )
						.then( ( _ret ) => {
							if ( customer.type_id ) {
